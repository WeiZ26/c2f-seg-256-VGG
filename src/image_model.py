...
class C2F_Seg(nn.Module):
    ...
    def get_losses(self, meta):
        ...
        # 生成 [B, num_patches] (即 [B, 256]) 的随机 mask
    ...
    def batch_predict_maskgit(self, meta, iter, mode, T=3, start_iter=0):
        ...
        # ========== 2. 创建全掩码输入 (新) ==========
        # 在推理时，我们没有真实 (ground truth) 的 fm_patches，
        # 所以我们传入一个全 0 的画布，让 Transformer 预测所有内容。
        fm_patches_input = torch.zeros_like(vm_patches)  # [B, 256, 256]
    
    
       # 1. Transformer 预测完整的像素 patches (一次性)
        pred_pixel_patches = self.transformer(
            img_feat[-1],
        ...
        # ... (align_raw_size, visualize, loss_and_evaluation ...)
