...
if __name__ == '__main__':
    ...
    args.local_rank = int(os.environ.get("LOCAL_RANK", args.local_rank))  # 添加这一行
    ...
    # 确保 g_path=None 来加载 image_model (MAE version)
    model = C2F_Seg(config, g_path=None, mode='test', logger=logger)
    model.load(is_test=True ,prefix = config.stage2_iteration)
    
    # MAE 版本不需要 restore_from_stage1
    ...
    with torch.no_grad():
        ...
        for items in test_loader:
            ...
            # [新增] 累加粗糙指标
            if 'iou_coarse' in loss_eval:
                iou_coarse += loss_eval['iou_coarse']
                invisible_iou_coarse += loss_eval['invisible_iou_coarse']
            ...
            # [修改] 更新日志打印
            logger.info('Rank {}, iter {}: iou_coarse: {:.4f}, iou: {:.4f}, iou_post: {:.4f}, occ_coarse: {:.4f}, occ: {:.4f}, occ_post: {:.4f}'.format(
                rank, 
                iter-1,
                loss_eval['iou_coarse'].item() if 'iou_coarse' in loss_eval else 0.0, # [新增]
                loss_eval['iou'].item(),
                loss_eval['iou_post'].item(),
                loss_eval['invisible_iou_coarse'].item() if 'invisible_iou_coarse' in loss_eval else 0.0, # [新增]
    ...
    # --- [修改 3：All Reduce 和最终日志] ---
    
    # [新增] All Reduce 粗糙指标
    dist.all_reduce(iou_coarse)
    dist.all_reduce(invisible_iou_coarse)
    ...
    # [修改] 增加最终的日志输出
    if rank==0:
        logger.info('meanIoU Coarse: {}'.format(iou_coarse.item() / (iou_count.item() + 1e-8))) # [新增]
        logger.info('meanIoU (Refined): {}'.format(iou.item() / (iou_count.item() + 1e-8))) # [修改描述]
        logger.info('meanIoU post-process: {}'.format(iou_post.item() / (iou_count.item() + 1e-8)))
        logger.info('meanIoU invisible Coarse: {}'.format(invisible_iou_coarse.item() / (occ_count.item() + 1e-8))) # [新增]
        logger.info('meanIoU invisible (Refined): {}'.format(invisible_iou_.item() / (occ_count.item() + 1e-8))) # [修改描述]
        logger.info('meanIoU invisible post-process: {}'.format(invisible_iou_post.item() / (occ_count.item() + 1e-8)))
